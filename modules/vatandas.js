...
function showDetail(isim, hizmet, ay) {
  ...
  document.getElementById('vk-body').innerHTML=`
    ...
    ${(()=>{const kb=(typeof kbData!=='undefined'?kbData:[]).find(x=>(x.AD_SOYAD||'').toUpperCase()===(r.ISIM_SOYISIM||'').toUpperCase())||{};const engVal=r.ENGEL||kb.ENGEL||'';const engYuz=r.ENGEL_YUZDE||kb.ENGEL_YUZDE||'';const engAc=r.ENGEL_ACIKLAMA||kb.ENGEL_ACIKLAMA||'';const varMi=engVal==='Var';const bg=varMi?'#fef2f2':'#f0fdf4';const border=varMi?'#fecaca':'#bbf7d0';const renk2=varMi?'#b91c1c':'#15803d';return`<div style="background:${bg};border:1.5px solid ${border};border-radius:10px;padding:10px 14px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;gap:10px"><div style="flex:1"><span style="font-size:10px;font-weight:800;color:#94a3b8;letter-spacing:.07em;text-transform:uppercase">Engel Durumu</span><br><span style="font-size:14px;font-weight:900;color:${renk2}">${varMi?'⚠️ Engelli':'✓ Engel Yok'}${engYuz?` — %${engYuz}`:''}</span>${engAc?`<br><span style="font-size:11px;color:#64748b;margin-top:2px;display:block">${engAc}</span>`:''}</div><span style="background:${renk2};color:#fff;border-radius:8px;padding:3px 10px;font-size:10px;font-weight:800;white-space:nowrap">${varMi?'ENGELLİ':'ENGEL YOK'}</span></div>`;})()}

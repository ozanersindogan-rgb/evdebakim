...
function normalizeRecord(r) {
  ...
  r.ISIM_SOYISIM = (r.ISIM_SOYISIM || '').toString().trim().toUpperCase();
  r.KISI_ID = (r.KISI_ID || '').toString().trim().toUpperCase();
  r.TC = (r.TC || '').toString().replace(/\D/g,'');
  return r;
}

function kisiAdNorm(v) {
  return String(v || '').trim().toLocaleUpperCase('tr-TR').replace(/\s+/g, ' ');
}
function kisiTelefonNorm(v) {
  return String(v || '').replace(/\D/g, '');
}
function kisiTarihNorm(v) {
...
function kisiKimlikAnahtari(rec) {
  if (!rec) return '';
  const tc = (rec.TC || '').toString().replace(/\D/g, '');
  if (tc && tc.length >= 11) return 'TC:' + tc;
  const tel = kisiTelefonNorm(rec.TELEFON || rec.tel || '');
  if (tel && tel.length >= 10) return 'TEL:' + tel;
  const isim = kisiAdNorm(rec.ISIM_SOYISIM || rec.AD_SOYAD || rec.isim || '');
  const dogum = kisiTarihNorm(rec.DOGUM_TARIHI || rec.dogum || '');
  if (isim && dogum) return 'ADDT:' + isim + '|' + dogum;
  return '';
}
function kisiAyniMi(a, b) {
  if (!a || !b) return false;
  if (a.KISI_ID && b.KISI_ID && a.KISI_ID === b.KISI_ID) return true;
  const ka = kisiKimlikAnahtari(a);
  const kb = kisiKimlikAnahtari(b);
  if (ka && kb && ka === kb) return true;
  const adA = kisiAdNorm(a.ISIM_SOYISIM || a.AD_SOYAD || a.isim || '');
  const adB = kisiAdNorm(b.ISIM_SOYISIM || b.AD_SOYAD || b.isim || '');
  if (!adA || !adB || adA !== adB) return false;
  const dA = kisiTarihNorm(a.DOGUM_TARIHI || a.dogum || '');
  const dB = kisiTarihNorm(b.DOGUM_TARIHI || b.dogum || '');
  if (dA && dB && dA === dB) return true;
  const tA = kisiTelefonNorm(a.TELEFON || a.tel || '');
  const tB = kisiTelefonNorm(b.TELEFON || b.tel || '');
  if (tA && tB && tA === tB) return true;
  return false;
}
function kisiIdUret() {
  const ids = new Set();
  (Array.isArray(allData) ? allData : []).forEach(r => { if (r?.KISI_ID) ids.add(r.KISI_ID); });
  if (Array.isArray(window.kbData)) window.kbData.forEach(r => { if (r?.KISI_ID) ids.add(r.KISI_ID); });
  const nums = [...ids].map(id => parseInt(String(id).replace(/\D/g,''), 10)).filter(Number.isFinite);
  const next = (nums.length ? Math.max(...nums) : 0) + 1;
  return 'K-' + String(next).padStart(6, '0');
}
function kisiMevcutKayitBul(aday) {
  const adayRec = normalizeRecord({ ...(aday || {}) });
  if (!adayRec) return null;
  if (adayRec.KISI_ID) {
    const byId = allData.find(r => r.KISI_ID === adayRec.KISI_ID);
    if (byId) return byId;
  }
  const byMaster = Array.isArray(window.kbData)
    ? window.kbData.find(k => kisiAyniMi(k, adayRec))
    : null;
  if (byMaster) return allData.find(r => r.KISI_ID === byMaster.KISI_ID) || byMaster;
  return allData.find(r => kisiAyniMi(r, adayRec)) || null;
}
function kisiIdBelirle(aday) {
  const bulunan = kisiMevcutKayitBul(aday);
  return bulunan?.KISI_ID || kisiIdUret();
}
async function kisiAltyapiKur(opts = {}) {
  const silent = !!opts.silent;
  try {
    const masterSnap = await firebase.firestore().collection('vatandaslar_bilgi').get();
    const masterDocs = [];
    const byKimlik = new Map();
    masterSnap.forEach(d => {
      const raw = d.data() || {};
      const rec = normalizeRecord({ _fbId: d.id, ...raw, AD_SOYAD: raw.AD_SOYAD || raw.ISIM_SOYISIM || '' });
      if (!rec.KISI_ID) rec.KISI_ID = kisiIdUret();
      masterDocs.push(rec);
      const key = kisiKimlikAnahtari(rec);
      if (key && !byKimlik.has(key)) byKimlik.set(key, rec);
    });

    const masterUpdates = [];
    masterDocs.forEach(m => {
      const raw = masterSnap.docs.find(x => x.id === m._fbId)?.data() || {};
      if ((raw.KISI_ID || '') !== (m.KISI_ID || '')) {
        masterUpdates.push(firebase.firestore().collection('vatandaslar_bilgi').doc(m._fbId).update({ KISI_ID: m.KISI_ID }));
      }
    });

    const recUpdates = [];
    for (const r of allData) {
      let kisiId = (r.KISI_ID || '').trim();
      if (!kisiId) {
        const key = kisiKimlikAnahtari(r);
        if (key && byKimlik.has(key)) kisiId = byKimlik.get(key).KISI_ID;
      }
      if (!kisiId) {
        const eslesen = masterDocs.find(m => kisiAyniMi(m, r));
        if (eslesen) kisiId = eslesen.KISI_ID;
      }
      if (!kisiId) {
        const onceki = allData.find(x => x !== r && kisiAyniMi(x, r) && x.KISI_ID);
        if (onceki) kisiId = onceki.KISI_ID;
      }
      if (!kisiId) kisiId = kisiIdUret();

      if (r.KISI_ID !== kisiId) {
        r.KISI_ID = kisiId;
        if (r._fbId) recUpdates.push(firebase.firestore().collection('vatandaslar').doc(r._fbId).update({ KISI_ID: kisiId }));
      }

      let m = masterDocs.find(x => x.KISI_ID === kisiId) || masterDocs.find(x => kisiAyniMi(x, r));
      if (!m) {
        const y = {
          KISI_ID: kisiId,
          AD_SOYAD: r.ISIM_SOYISIM || '',
          MAHALLE: r.MAHALLE || '',
          TELEFON: r.TELEFON || '',
          ADRES: r.ADRES || '',
          ...
          TC: r.TC || '',
          HIZMET: r['HİZMET'] || '',
          HIZMETLER: [...new Set(allData.filter(x => kisiAyniMi(x, r)).map(x => x['HİZMET']).filter(Boolean))]
        };
        masterDocs.push(y);
        recUpdates.push(firebase.firestore().collection('vatandaslar_bilgi').add(y));
      }
    }

    const chunks = arr => {
      const out = [];
      for (let i = 0; i < arr.length; i += 300) out.push(arr.slice(i, i + 300));
      return out;
    };
    for (const part of chunks(masterUpdates)) await Promise.all(part);
    for (const part of chunks(recUpdates)) await Promise.all(part);

    if (!silent && (masterUpdates.length || recUpdates.length)) {
      showToast(`✅ Kişi sicil altyapısı güncellendi (${masterUpdates.length + recUpdates.length} işlem)`);
    }
    return true;
  } catch (e) {
    console.error('kisiAltyapiKur hatası:', e);
    if (!silent) showToast('❌ Kişi sicil altyapısı kurulamadı: ' + e.message);
    return false;
  }
}
...
async function fbLoadData() {
  ...
  try {
    ...
    await kisiAltyapiKur({ silent: true });
